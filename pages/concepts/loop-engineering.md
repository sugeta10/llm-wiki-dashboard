...
- 2026-06-08 @mvanhorn: Uber が Claude Code と Cursor の1人あたり月額上限を $1,500 に設定後、年間AI予算を4ヶ月で使い切ったと報告。「ループを書いた後に最も高価なのはトークンでなくループ管理そのもの」（単一ソース）
- 2026-06-08 @mvanhorn: 成熟したループには3つのハードストップが必要——最大イテレーション数・無進捗検出・ドルまたはトークンの予算上限。Gartner はエージェントAIをインフレーション期のピークに位置付け、実際にデプロイしている組織は約17%と主張
- 2026-06-08 @mvanhorn: Steve Yegge の Gas Town（2026年1月）: Mayor エージェント下で20〜30の Claude Code インスタンスが協調。パトロールエージェントが継続ループを実行し、git にステートを保存してクラッシュサバイバルを実現（オープンソース・著者主張）
- 2026-06-08 @mvanhorn: ralph loop は Geoffrey Huntley が2025年7月に公開した bash ワンライナー。同一プロンプトファイルをエージェントに繰り返しパイプするだけだが、イテレーションごとにコンテキストをアンカーファイルにリセットする規律が革新。約 $297 でプログラミング言語を構築したと主張
- 2026-06-08 @addyosmani: ループが変えるのは「仕事がなくなる」ではなく「仕事の重心が移動する」。Verification（検証）・comprehension debt（理解の劣化）・cognitive surrender（思考の放棄）という3つの問題はループが自動化されるほど先鋭化すると指摘
- 2026-06-09 @masa_okamura108: Codex /goal を活用した Notion タスクAI自動実行ループで、1日で43件のコミットを生成。しかし修正範囲が元のPRの趣旨から逸脱し、ほぼ全件却下。「ループの設計ミス→暴走→トークン爆増」の実例として紹介
- 2026-06-09 @masa_okamura108: 不要ブランチ削除ループは即効性があり129件の不要ブランチを削除（著者の実測値・未検証）
- 2026-06-11 @steipete: codexにリポジトリのメンテを任せ、5分ごとに起き上がって複数スレッドへ作業を割り振るループを回していると報告。orchestratorスキルをtriage・autoreview・computer useの各スキルと組み合わせることで一部の作業はそのままマージまで到達すると述べる。Stage 5（ループがループを監視・起動するオーケストレーション）の具体例であり、「ループが鋭い名前付きスキルを呼ぶ」構成の実践（単一ソース・[[tools/autoreview-skill]] / [[tools/skill-cleaner]] と同じ作者）

- 2026-06-09 @0xCodez（二次まとめ）: Anthropicエンジニアが1日に8倍のコードをマージするようになったとAnthropicが報告——ただし同社自身が「真の生産性向上の過大評価の可能性が高い」と注記。ループの効果は実在するが数字は議論中
- 2026-06-09 @0xCodez: Ralph Wiggumループ（Geoffrey Huntley命名）: 完了トークンの早期出力でループが中途半端な状態で静かに終了する失敗モード。命名の由来はSimpsonsのキャラクター
- 2026-06-09 @0xCodez: コミュニティ監査で17,022スキル中520件が認証情報を漏洩する可能性があると指摘——コミュニティスキルの自動インストールは危険（未検証）
- 2026-06-09 @0xCodez: 本記事の結論: 「多くの開発者はまだループを必要としていない」——4条件を満たさない間は良いプロンプトが勝つ。Chernyの言葉を引用: 「仕事が楽になったのではなく、レバレッジの点が移動した。ループを作れ。エンジニアであり続けろ」
- 2026-06-09 @sairahul1: Peter Steinberger（OpenClaw創設者・現OpenAI）とBoris Cherny（Claude Code責任者）が独立して「プロンプトを書くな、ループを書け」という同一メッセージを発信。著者はこれを「地球上で最も重要な2つのAIラボのトップエンジニアが同一パターンに収束している」シグナルとして深掘り解説を公開
- 2026-06-09 @sairahul1: トークンコスト試算（著者推定値）: シングルエージェントループ（中規模タスク）5万〜20万トークン、フリートループ（オーケストレーター+スペシャリスト3体）50万〜200万トークン、週次スケジュール実行で数百万トークン規模。DeepSeek V4の低価格で「$20で約1.7Bトークン」が利用でき経済的ブロッカーが解消するとの主張（著者試算・未検証）
- 2026-06-11 @koutarou_furuno（@sairahul1記事の日本語二次まとめ）: ループ工学の本質を「指示を改善するレイヤー」から「指示を出す仕組みを設計するレイヤー」へのレイヤー上昇として整理。DeepSeek採用にはデータガバナンス・セキュリティの事前確認が必要と警告（ビジネス用途）
- 2026-06-16 @sydneyrunkle（LangChain）: ループエンジニアリングの焦点を L1・L2（エージェント＋検証）から L3・L4（イベント駆動＋ヒルクライミング）へ移すべきと主張——価値が複利で積み上がるのは、エージェントをエコシステムに埋め込み基準に応じて継続改善させる外側の層だから。@swyx の「loopcraft（loopを積み重ねる技術）」を引用し、Steipete・Boris・Karpathy が独立に「エージェントの潜在力はその周りに作るループにある」という同一結論へ収斂していると指摘（LangChain公式ブログ・自社プリミティブ前提・単一ソース）
- 2026-06-16 @sydneyrunkle（LangChain）: Satya Nadella のフレーミングを引用——「学習ループを早期に作り、人間の判断とトークン資本を複利させる企業が、模倣困難な優位を築く」。[[business/frontier-ecosystem]] と同一主張の別ルート裏付け
- 2026-06-19 @omarsar0（Elvis Saravia/DAIR.AI・本記事はClaude併用で執筆）: 良い `/goal` は「プロンプトより契約に近い」——終了状態・到達を証明する根拠・破ってはならない制約・許される作業予算の4点を明示すべきと整理。どれか1つでも曖昧だとモデルが最も楽な解釈で埋め、早期停止・ショートカット・成功の再定義が起きると指摘（著者の整理）
- 2026-06-19 @omarsar0: Cherny の「Opusを数時間自律運用する」設定を5ステップに整理——①権限の自動承認 ②動的ワークフロー（プロンプトにUltracode投入）で多エージェントへfan-out ③/goal or /loopで継続（両者とも状態を持ち越す）④クラウド（デスクトップ/モバイルアプリ）実行でラップトップを閉じてもセッションを生存させる ⑤エンドツーエンドの自己検証手段を与える（Web=Claude in Chrome / モバイル=シミュレータMCP / バックエンド=ライブサーバー）。⑤が他4つを安全にする鍵だと述べる（著者によるCherny発言の整理）
- 2026-06-19 @omarsar0: コストの所在が「どのモデル・何トークン」から「ループが何周回るか」へ移ったと指摘。①最適化対象はトークンでなくイテレーション数（cost per finished task=完了タスクあたりコストで測れ）②弱い検証器は最も高価なバグ（緩いと早期停止か無駄な周回でイテレーションを丸ごと浪費）③fail fast はコスト管理（連続失敗の上限がないループは成功でなく口座を空にする）（著者の整理）
