...
## ChatGPT公式プロンプティングガイド（Goal/Context/Output/Boundaries）

OpenAI公式のプロンプティングドキュメント（learn.chatgpt.com/docs/prompting）が示す、Chat・Work・Codexに共通する4要素フレームワーク。

- **Goal（目的）**: ChatGPTに何をしてほしいか
- **Context（文脈）**: 何の情報・ソースが助けになるか
- **Output（出力）**: どの形式・長さ・詳細度が必要か
- **Boundaries（境界）**: 何を変えてはいけないか、何を避けるべきか、何を事前確認すべきか

OpenAI公式は「4項目すべてを毎回埋める必要はなく、結果を左右する要素だけ書けばよい」と説明する。短いプロンプトで十分な場合も多いが、規模の大きいタスクや重要なタスクほど各要素を明示する価値が上がる、という位置づけ。

境界（Boundaries）は「変更したら成果物が使い物にならなくなる詳細」や「他者に影響する前にレビューしたい行動」がある場合にのみ追加する。1〜2個に絞り、すべての手順を縛ろうとしないのが要点。

**Work（時間のかかる複雑タスク）向けの追加指針**: ソース資料・対象読者・レビュー方法を明示したうえで、計画→情報収集→ファイル作成→自己チェックをChatGPTに任せる。最初の1回で完璧を狙わず、レビュー結果を見てから指示を絞り込み、うまくいったワークフローを再利用する運用が推奨されている。

**フォローアップでの改善**: 最初のプロンプトを完璧に作る必要はなく、結果を見てから「この変更だけ」と絞って追加指示を出す運用が前提。Codex CLIでは実行中のターンに割り込む「Steer」と次のターン用に指示を貯める「Queue」が区別されている。
