...
デジタル庁が、行政手続等調査データ（約75,000件）を[[tools/claude-mcp]]経由で対話的に自然言語分析する実装をGitHubで公開し、Techブログで解説している。同庁は技術検証目的の簡易的な実装と位置づけたうえで、**AIの誤った補完・推測を減らす**ことと、MCP AppsによるチャットUI表示（[[concepts/generative-ui]]のOpen-endedパターンにあたる）の試作を狙いとして挙げている。本ページが扱うエンティティ曖昧性への対処を、社内データではなく公開された行政データで実演した事例にあたる。

本Wikiが取り込んだのは@keitaro_aigcによる紹介ポストで、デジタル庁のTechブログ本文とリポジトリは未読である。データ基盤・情報源・スキルの各層をどう作っているかは未確認のため、上記以外の設計内容は本ページでは扱わない。
...
- デジタル庁の公開実装は、本ページの4層のうちどこまでを実装しているか。公開リポジトリを読めば「セマンティック層を人間が所有する」という原則が公共データでどう実現されているか確認できるはず
...
- [[tools/claude-mcp]] — デジタル庁の実装が分析インターフェースとして採用したプロトコル
- [[concepts/generative-ui]] — デジタル庁が試作したMCP AppsによるチャットUIの位置づけ
