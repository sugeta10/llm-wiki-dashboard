...
中国語圏では @Pluvio9yte が「Jev の API キーを手に入れたが遊び方が分からない人向け」の真似できるリストの1番目に本リポジトリを挙げ、Jev は各ステップで「何をするか・どの要素を押すか」だけを判断し、文字入力のときだけ小型モデルを呼ぶ、Google Flights の検索は約7秒と紹介している（ブックマーク3,218件）。リストの2番目以降は捕捉が途切れている（[[concepts/jev-claude-code-integration]] 参照）。
...
- [[concepts/decision-layer-model]] — 本ページの「観測ごとに選択肢を作り直す」設計を、司令塔エージェントのワーカー選択へ一般化した判断レイヤーの設計論
